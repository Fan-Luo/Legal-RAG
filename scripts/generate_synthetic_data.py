...
def has_mixed_lang(text: str) -> bool:
    ...
    if not text:
        return False
    has_cn = any(is_chinese_char(ch) for ch in text)
...
def is_overly_abstract(text: str) -> bool:
    ...
    if ABSTRACT_PATTERN.search(text):
        if not any(k in text for k in CN_FACT_ANCHORS) and not any(k in text.lower() for k in EN_FACT_ANCHORS):
            return True
...
def has_min_fact_anchor(text: str) -> bool:
    if not text:
        return False
    t = text.strip()
    if any(k in t for k in CN_FACT_ANCHORS):
        return True
    tl = t.lower()
...
def generate_queries_for_article(
    ...
    # -------- 单轮生成 --------
    for role in roles:
        lang = "cn" if random.random() < cn_ratio else "en"
...
def main():
    ...
    logger.info(f"Seed={args.seed}, cn_ratio={args.cn_ratio}")
