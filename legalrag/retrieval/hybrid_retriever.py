...
class HybridRetriever:
    ...
    def search(self, question: str, top_k: int = 10, decision: Any = None) -> List[RetrievalHit]:
        ...
        eff_top_k = int(getattr(rcfg, "candidate_k", top_k * 8) or (top_k * 8))
        if eff_top_k < top_k:
            eff_top_k = top_k
        ...
        # optional rerank
        if self.reranker is not None and getattr(rcfg, "enable_rerank", False):
            ...
            if docs:
                try:
                    if hasattr(self.reranker, "scores"):
                        r_raw = [float(x) for x in self.reranker.scores(question, docs)]  # type: ignore[attr-defined]
                    else:
                        r_raw = [float(self.reranker.score(question, d)) for d in docs]  # type: ignore[attr-defined]
                except Exception:
                    r_raw = [0.0 for _ in docs]
